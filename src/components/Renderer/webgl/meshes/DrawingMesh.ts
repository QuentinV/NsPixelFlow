...
import { DrawingMeshSettings } from '../../../../api/projects';
import { BaseMesh, MeshProps } from './types';

export class DrawingMesh extends BaseMesh<DrawingMeshSettings> {
    increaseDetails: number;

    constructor(props: MeshProps<DrawingMeshSettings>) {
        super(props);

        this.increaseDetails = this.settings().increaseDetails ?? 0;
        this.points = null;
    }
    ...
    createDrawing(contours, nextContours) {
        ...
        if (this.options.increaseDetails) {
            convertedContours = convertedContours.map((cp) =>
                this.resamplePoints(
                    cp,
                    cp.length * this.options.increaseDetails
                )
            );
        }
        ...
        if (nextContours) {
            ...
            if (this.options.increaseDetails) {
                nextContours = nextContours.map((cp) =>
                    this.resamplePoints(
                        cp,
                        cp.length * this.options.increaseDetails
                    )
            ...
            convertedContours = convertedContours
                .map((cp, index) => {
                    const targetCount = nextContours[index]?.length;
                    return targetCount
                        ? this.resamplePoints(cp, targetCount)
                        : null;
                })
                .filter((cp) => !!cp);
        }
    ...
    create(k?: number, nextContours?: any) {
        ...
        return this.createDrawing(
            this.options.drawings[k].points,
            nextContours
        );
    }
